...
- 作为 Card Catalog 定位 Card Definition 的查询键。

模拟器内部将 Card Number 保存为结构化数据。其基本组成如下：
...
- `variant_code` 为可选的印刷 / 表现版本代码。它可以用于区分同一基础 Card 的不同版本及 UI 资源，但不改变该 Card 的规则身份。

由 `title_code`、`side`、`product_code` 与 `product_card_number` 生成的编号称为 `base_card_number`。加入可选 `variant_code` 后得到 `full_card_number`。

Card 层不负责依据这些子字段进行 Deck 合法性判断。Card Number 的结构化表示、存储定位、Card Catalog 查询及规则层用途详见 **附录 A：Card Number 与 Card Catalog**。

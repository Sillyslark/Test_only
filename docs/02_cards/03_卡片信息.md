...
| 卡片编号 | Card Number | `card_number` | `card_number` | 已实现 | 保存基础 Card Number |
...
Card Number 是用于区分并定位 Card Definition 的固有编号。

在 Card 层，Card Number 的职责仅限于：

- 唯一区分基础 Card Definition；
- 作为 Card Index 定位 Card Definition 的查询依据。

模拟器内部将 Card Number 保存为结构化数据。基础 Card Definition 中保存：
...
测试卡使用模拟器保留的测试用 Title Code `TEST`。例如：
...
- `title_code` 对应作品番号 / Title Code；
- `side` 表示 Weiß / Schwarz 等 Side 信息；
- `product_code` 表示该 Card 所属商品或收录批次的内部代码；
- `product_card_number` 表示同一商品或收录批次中用于区分具体基础 Card 的编号。

由 `title_code`、`side`、`product_code` 与 `product_card_number` 生成的编号称为 `base_card_number`。

`variant_code` 为可选的印刷 / 表现版本代码。加入可选 `variant_code` 后得到 `full_card_number`。`variant_code` 用于区分同一基础 Card 的不同版本及 UI 资源，但不改变该 Card 的规则身份，也不属于基础 Card Definition 保存的 Card Information。

Card 层不负责依据这些子字段进行 Deck 合法性判断。Card Number 的结构化表示、存储定位、Card Index 查询及规则层用途详见 **附录 A：Card Number 与 Card Index**。
